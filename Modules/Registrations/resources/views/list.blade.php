...
<div class="bg-gray-50">
    <div class="container mx-auto   ">
        <div class="mb-10  ">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Your School Registrations</h1>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden card-hover border border-gray-100">
                <div class="p-6">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="flex items-start space-x-4">
                            ...
                            <div>
                                <div class="flex items-center space-x-2 mb-1">
                                    <h3 class="text-lg font-semibold text-gray-900">Registration #{{ $form->id }}</h3>
                                ...
                                <p class="text-gray-600">
                                    ...
                                    {{ json_encode($form->data['sectionA']['proposed_name'] ?? 'Incomplete Form') }}  
                                    <div class="  flex items-center space-x-2"> Current : {{optional($form->currentApproval()->stage ?? null)->name ?? 'N/A' }} -> 
                                    <a href="{{ route('registration.timeline', ['form_id' => $form->id ?? 'default']) }}" 
                               >
                        ...
                        <div class="flex space-x-3">
                            ...
                            <a href="{{ route('registration.portfolio', ['id' => $form->id ?? 'default']) }}" 
                               class="btn-outline flex items-center space-x-2">
                                <i class="far fa-eye"></i>
                                <span>Portfolio</span>
                            </a>
                            
                            <!--<a href="{{ route('registration.preview', ['form_id' => $form->id ?? 'default']) }}" 
                               class="btn-outline flex items-center space-x-2">
                                <i class="far fa-eye"></i>
                                <span>Summary</span>
                            </a>-->
                            <a href="{{ route('registration.sectionA.show', ['form_id' => $form->id ?? 'default']) }}" 
                ...
                @if($form->pendingApprovalPay!=null)
                <div class="p-6"><a href="{{route('registration.pay.approval',['id'=>$form->id])}}" class="btn-outline flex items-center space-x-2">
                                <i class="far fa-bank"></i>
                                <span>Pay Approval Fee</span></a></div>
                @endif
...
/*
@php
@if(isset($form->currentApproval()))
                     $perc=number_format((($form->currentApproval()->stage->order-1 ?? 1)/7)*100,2);
@else
$perc=100;
@endphp 
*/
