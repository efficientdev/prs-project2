<div class="my-2">PROPOSED STAFFING</div>
...
<script>
    function staffForm() {
        return {
            staffList: [
                @php
                    $oldStaff = collect(old('staff_surname', []));
                    $count = max($oldStaff->count(), 1);
                    for ($i = 0; $i < $count; $i++) {
                        echo json_encode([
                            'surname' => old("staff_surname.$i"),
                            'otherNames' => old("staff_other_names.$i"),
                            'qualification' => old("staff_qualification.$i")
                        ]) . ($i < $count - 1 ? ',' : '');
                    }
                @endphp
            ],
