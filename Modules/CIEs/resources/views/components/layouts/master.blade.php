...
@php
            $sections = [
                ...
                'sectionB' => 'Enrolment Data',
                'sectionC' => 'Staffing Data',
                'sectionD' => 'Infrastructure & Facilities',
                //'sectionE' => 'Fees ',
                //'sectionF' => 'Required Documents Upload',
                'sectionG' => 'Observations & Recommendations',
            ];
        ...
        @endphp
                ...
                @if(auth()->user()->hasAnyRole(['CIE','ADM']) )
 

<a href="{{route('registration.list')}}">Home</a> 

@foreach($sections as $key => $label)
                <a href="{{ route('cies.' . $key . '.show', ['report' => $form_id ?? 'default']) }}" class="...">
                    {{ $label }}
                </a>
            @endforeach
                ...
                <div class="flex h-full flex-1 z-1  "> 
                    ...
                    <main class="flex-1 overflow-y-auto  px-0 pt-[100px] mb-1 mr-1 px-2 md:pr-5 min-h-screen">
...
<div class="text-3xl my-2"> CIE Inspection Form</div> 
<hr class="my-2" />
