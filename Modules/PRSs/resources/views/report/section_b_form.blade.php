...
$data=$sectionB;
@endphp
<form method="POST" action="{{ route('prss.sectionB.store', $report->id) }}">
    @csrf
        ...
        <table class="table-auto w-full border mb-4">
            ...
            <tbody>
                <template x-for="(row, i) in rows" :key="i">
                    <tr>
                        <td class="border p-1"  >
                            <input type=""  :name="`levels[${i}][level]`" x-model="row.level" class="w-full border-none bg-transparent" readonly="">
                        </td>
                        <td class="border p-1"  >
                            <input type="text"  :name="`levels[${i}][male]`" x-model.number="row.male" class="w-full border-none bg-transparent" readonly="" >
                        </td>
                        <td class="border p-1">
                            <input type="number" readonly :name="`levels[${i}][female]`" x-model.number="row.female" class="w-full border-none bg-transparent" readonly="">
                        </td>
    ...
    <div class="flex justify-between">
        <a href="{{ route('prss.sectionA.show', $report->id) }}" class="btn bg-gray-500 text-white px-6 py-2 rounded">Previous</a>
