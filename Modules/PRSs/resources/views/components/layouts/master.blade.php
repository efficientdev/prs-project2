...
@php
        ...
        @endphp
                ...
                @if(auth()->user()->hasAnyRole(['PRS','ADM']) )
 

<a href="{{route('registration.list')}}">Home</a> 

@foreach($sections as $key => $label)
                <a href="{{ route('prss.' . $key . '.show', ['report' => $form_id ?? 'default']) }}" class="...">
                    {{ $label }}
                </a>
            @endforeach
...
<div class="text-3xl my-2"> PRS Inspection Form</div> 
<hr class="my-2" />
