...
@php
            $sections = [
                ...
                'sectionB' => 'Student Enrolment',
                'sectionC' => 'Staff Profile',
                'sectionD' => 'Infrastructure',
                'sectionE' => 'Validation & Compliance ',
                'sectionF' => 'Remarks',
...
<a href="{{route('public.validation.list')}}">Home</a> 

@foreach($sections as $key => $label)
                <a href="{{ route('public.validation.' . $key . '.show', ['form_id' => $form_id ?? 'default']) }}" class="...">
                    {{ $label }}
                </a>
            @endforeach
...
<div class="text-3xl my-2">Public School Validation Form</div> 
